...
def create_buttons_from_files(directory, button_template_name, text_template_name, parent_obj, collection, kxscene, name_prefix=""):
    y_offset = 0
    for filename in os.listdir(directory):
        if filename.endswith(".json") or filename.endswith(".blend"):
            # Duplicar el botón base con nombre único
            button = duplicate_and_prepare_object(
                bpy.context.scene.objects.get(button_template_name),
                collection, kxscene,
            ...
            # Duplicar el texto base
            text_obj = bpy.data.objects.get(text_template_name)
            if not text_obj:
                print(f"No se encontró el objeto texto '{text_template_name}'.")
                continue

            text_copy = duplicate_and_prepare_object(
                text_obj,
                collection,
                kxscene,
                f"text:{name_prefix}_{filename}"
            )
            text_copy.setParent(button)
            text_copy.position = (0, 0, 0.01)
            ...
            # Asignar como texto el nombre del archivo sin extensión
            text_copy["Text"] = os.path.splitext(filename)[0]

            print(f"Botón creado para: {filename} en {button.position}")
